...
def determineBadlandlordsFromViolationsDatasets():
    ...
    sam_df = fetchSamData()
    violations_df = getParcelIdsOfBuildingWithViolations(violations_df, sam_df)
    fetchPropertyData(violations_df)

def fetchSamData():
    # Sam dataset is not available in via api currently so 
...
def getParcelIdsOfBuildingWithViolations(violations_df, sam_df):
    """
...
def fetchPropertyData(violations_df):
    ...
    violation_parcels = tuple(violations_df['parcel'].unique()) # we only need the lanlords of the properties with violations
    violation_parcels_str = '"' + violation_parcels + '"'  # add quotes around the tuple
    url = f"https://data.boston.gov/api/3/action/datastore_search_sql?sql=SELECT * from \"a9eb19ad-da79-4f7b-9e3b-6b13e66f8285\" WHERE \"OWN_OCC\" = 'N' AND PID IN {violation_parcels_str}"
    print(url)
